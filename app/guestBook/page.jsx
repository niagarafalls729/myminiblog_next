    ...
    try {
      ...
      if (response.data && response.pagination) {
        ...
        const transformedData = response.data.map(item => {
          ...
          // PC에서만 제목에 댓글수 추가 (모바일은 원래대로 배지로 표시)
          if (!isMobile && transformedItem.COMMENT_COUNT > 0) {
            transformedItem.TITLE = `${transformedItem.TITLE} (${transformedItem.COMMENT_COUNT})`;
          }
